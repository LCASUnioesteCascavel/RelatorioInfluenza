...
Humanos::Humanos(Parametros *parametros, Ambiente *ambiente) {
  ...
  // Criacao e inicializacao dos agentes humanos. 
  contarHumanos();
...
void Humanos::atualizacaoIndices() {
  ...
  // Ordenacao dos agentes humanos por quadra. 
  sort(
    humanosDev->begin(), humanosDev->end(), 
    LessQuadraHumano()
  );

  // Conversao dos agentes humanos para identificadores de quadra. 
  ...
  // Sao obtidas as quantidades de agentes por quadra. 
  reduce_by_key(
    k1.begin(), k1.end(), v1, k2.begin(), v2.begin()
  );

  // Para as quadras sem agentes sao inseridos no vetor de indices elementos 
  // com valor zero. 
  ...
  // Realiza uma soma parcial para obter os indices para as quadras. 
  inclusive_scan(
...
void Humanos::inserirHumanos(int quantidade, int estado, int sexo, 
  ...
  for (int j = 0; j < quantidade; ++j) {
    ...
    // Inicialmente os agentes nao podem estar posicionados nas ruas. 
    if (q == RUA) {
      j--;
      continue;
    }

    // Escolhe aleatoriamente uma posicao inicial para o agente. 
  ...
  for (int sexo = MASCULINO; sexo <= FEMININO; ++sexo) {
    for (int idade = BEBE; idade <= IDOSO; ++idade) {
      for (int estado : {SUSCETIVEL, EXPOSTO, INFECTANTE, RECUPERADO}) {
        // Percorre todos os parametros de insercao de humanos em 
        // "Entradas/MonteCarlo_{1}/Humanos/0-INI.csv". 
        inserirHumanos(parametros->parametros[desl], estado, sexo,
                        idade, i);
        // E somado dois pois cada parametro tem um valor minimo e maximo. 
        desl += 2;
      }
    }
  }
